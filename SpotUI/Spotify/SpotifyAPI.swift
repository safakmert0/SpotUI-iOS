...
enum SpotifyAPI {
    ...
    static func me() async throws -> SpotifyUser {
        do {
            let response: JSONObject = try await gqlRequest(operation: "profileAttributes")
            let data = response["data"] as? JSONObject
            let me = data?["me"] as? JSONObject
            guard let profile = me?["profile"] as? JSONObject else {
            ...
            return SpotifyUser(
                ...
                email: nil,
                images: parseGqlImages(profile?["avatar"]?["sources"] as? [Any]),
                product: nil,
                country: nil
            )
        } else {
    ...
    static func lyrics(trackId: String) async throws -> SpotifyLyrics {
        ...
        let data = try await client.getData(urlString, headers: ["Authorization": "Bearer \(token)"])
        let root = try parseJSONObject(from: data)
        ...
        let lines = (lyricsObj["lines"] as? [JSONObject])?.compactMap { line -> SpotifyLyricLine? in
            ...
            let startMs = Int64(line["startTimeMs"] as? String ?? "0") ?? 0
            return SpotifyLyricLine(startMs: startMs, words: words)
        } ?? []
    ...
    static func myPlaylists(limit: Int = 50, offset: Int = 0) async throws -> SpotifyPaging<SpotifyPlaylist> {
        ...
        guard let libraryData = json(response, "data", "me", "libraryV3") else {
            throw SpotifyError(statusCode: 500, message: "Invalid libraryV3 response")
        }
        let totalCount = libraryData["totalCount"] as? Int ?? 0
        let rawItems = libraryData["items"] as? [Any] ?? []
        let playlists = rawItems.compactMap { item -> SpotifyPlaylist? in
            guard let wrapper = item["item"] else { return nil }
            guard let typeName = wrapper["__typename"] as? String,
                  typeName == "PlaylistResponseWrapper" || typeName.lowercased().contains("playlist") else { return nil }
    ...
    static func playlist(_ playlistId: String) async throws -> SpotifyPlaylist {
        ...
        guard let playlist = json(response, "data", "playlistV2") else {
            throw SpotifyError(statusCode: 500, message: "Invalid fetchPlaylist response")
        }
        let ownerData = json(playlist as? JSONObject, "ownerV2", "data")
        let ownerUri = ownerData?["uri"] as? String ?? ""
        return SpotifyPlaylist(
            id: playlistId,
            name: playlist["name"] as? String ?? "",
            description: playlist["description"] as? String,
            images: parseGqlPlaylistImages(playlist["images"]),
            owner: SpotifyPlaylistOwner(
                id: String(ownerUri.split(separator: ":").last ?? ""),
                displayName: ownerData?["name"] as? String,
                uri: ownerUri.isEmpty ? nil : ownerUri
            ),
            tracks: SpotifyPlaylistTracksRef(total: jsonInt(json(playlist as? JSONObject, "content"), "totalCount") as? Int ?? 0),
            uri: "spotify:playlist:\(playlistId)",
            public: nil,
            collaborative: false,
            snapshotId: nil
    ...
    static func playlistTracks(playlistId: String, limit: Int = 100, offset: Int = 0) async throws -> SpotifyPaging<SpotifyPlaylistTrack> {
        ...
        let response = try await gqlRequest(operation: "fetchPlaylist", variables: vars)
        guard let content = json(response, "data", "playlistV2")?["content"] else {
            throw SpotifyError(statusCode: 500, message: "No content in fetchPlaylist response")
        }
        let tracks = (content["items"] as? [Any] ?? []).compactMap { elem -> SpotifyPlaylistTrack? in
            guard let itemWrapper = elem["itemV2"], let itemData = itemWrapper["data"] else { return nil }
    ...
    static func likedSongs(limit: Int = 50, offset: Int = 0) async throws -> SpotifyPaging<SpotifySavedTrack> {
        ...
        guard let tracksData = json(response, "data", "me", "library", "tracks") else {
            throw SpotifyError(statusCode: 500, message: "Invalid fetchLibraryTracks response")
        }
        let savedTracks = (tracksData["items"] as? [Any] ?? []).compactMap { elem -> SpotifySavedTrack? in
            guard let trackWrapper = elem["track"], let trackData = trackWrapper["data"] else { return nil }
    ...
    static func addToLibrary(uris: [String]) async throws {
        let vars: JSONObject = ["libraryItemUris": uris]
        _ = try await gqlRequest(operation: "addToLibrary", variables: vars)
    }

    static func removeFromLibrary(uris: [String]) async throws {
        let vars: JSONObject = ["libraryItemUris": uris]
        _ = try await gqlRequest(operation: "removeFromLibrary", variables: vars)
    }

    // MARK: - Album

    static func album(_ albumId: String) async throws -> SpotifyAlbum {
        return try await restGet("albums/\(albumId)")
    }

    static func albumTracks(albumId: String, limit: Int = 50, offset: Int = 0) async throws -> SpotifyPaging<SpotifyTrack> {
        let response: SpotifyPaging<SpotifyTrack> = try await restGet("albums/\(albumId)/tracks?limit=\(limit)&offset=\(offset)")
        return response
    }

    // MARK: - Artist

    static func artist(_ artistId: String) async throws -> SpotifyArtist {
        return try await restGet("artists/\(artistId)")
    }

    static func artistTopTracks(artistId: String, market: String = "US") async throws -> [SpotifyTrack] {
        let response: JSONObject = try await restGetRaw("artists/\(artistId)/top-tracks?market=\(market)")
        return (response["tracks"] as? [JSONObject] ?? []).compactMap { try? JSONDecoder().decode(SpotifyTrack.self, from: JSONSerialization.data(withJSONObject: $0)) }
    }

    static func artistAlbums(artistId: String, limit: Int = 20) async throws -> SpotifyPaging<SpotifyAlbum> {
        return try await restGet("artists/\(artistId)/albums?limit=\(limit)&include_groups=album,single,compilation")
    }
    ...
    static func recommendations(seedTracks: [String], limit: Int = 20) async throws -> SpotifyRecommendations {
        let seeds = seedTracks.prefix(5).joined(separator: ",")
        return try await restGet("recommendations?seed_tracks=\(seeds)&limit=\(limit)")
    }

    // MARK: - Track

    static func track(_ trackId: String) async throws -> SpotifyTrack {
        return try await restGet("tracks/\(trackId)")
    }
    ...
    private static func gqlRequest(operation: String, variables: JSONObject = [:]) async throws -> JSONObject {
        guard let token = accessToken else {
            throw SpotifyError(statusCode: 401, message: "Not authenticated")
        }
        let body: JSONObject = [
            "variables": variables,
            ...
            "extensions": [
                "persistedQuery": [
                    "version": 1,
                    "sha256Hash": SpotifyHashProvider.hash(for: operation) ?? "",
                ]
            ]
        ]
        let bodyData = try JSONSerialization.data(withJSONObject: body)
        return try await client.post(gqlURL, body: bodyData, headers: [
    ...
    private static func restGet<T: Decodable>(_ endpoint: String) async throws -> T {
        ...
        return try await client.get(urlString, headers: [
            "Authorization": "Bearer \(token)",
            "User-Agent": randomUserAgent(),
            "app-platform": "WebPlayer",
            "Origin": "https://open.spotify.com",
    ...
    private static func restGetRaw(_ endpoint: String) async throws -> JSONObject {
        guard let token = accessToken else {
            throw SpotifyError(statusCode: 401, message: "Not authenticated")
        }
        let urlString = restBase + endpoint
        let data = try await client.getData(urlString, headers: [
            "Authorization": "Bearer \(token)",
            "User-Agent": randomUserAgent(),
        ])
    ...
    private static func randomUserAgent() -> String {
        ...
        let chromeMajor = 140 - Int.random(in: 0...4)
        let chromePatch = Int.random(in: 0...499)
        let os = osOptions.randomElement()!
    ...
    private static func parseGqlPlaylistImages(_ imagesObj: Any?) -> [SpotifyImage] {
        guard let obj = imagesObj as? [String: Any],
    ...
    private static func parseGqlTrack(_ trackData: JSONObject, uriOverride: String? = nil) -> SpotifyTrack? {
        ...
        let artistsArr = trackData["artists"] as? JSONObject
        let artistsItems = artistsArr?["items"] as? [[String: Any]] ?? []
        let artists = artistsItems.compactMap { elem -> SpotifySimpleArtist? in
            guard let uri = elem["uri"] as? String else { return nil }
            let profile = elem["profile"] as? JSONObject
        ...
        let coverArt = albumData?["coverArt"] as? JSONObject
        let album = SpotifySimpleAlbum(
            id: String(albumUri.split(separator: ":").last ?? ""),
            name: albumData?["name"] as? String ?? "",
            images: parseGqlImages(coverArt?["sources"]),
            uri: albumUri.isEmpty ? nil : albumUri
        )
        let durationObj = trackData["duration"] as? JSONObject
        let durationMs = durationObj?["totalMilliseconds"] as? Int
            ?? trackData["durationMs"] as? Int
            ?? trackData["duration_ms"] as? Int
            ?? 0
        return SpotifyTrack(
            id: trackId,
            ...
            uri: uri.isEmpty ? nil : uri
        )
    }

    private static func parsePlaylistWrapper(_ wrapper: JSONObject) -> SpotifyPlaylist? {
        guard let data = wrapper["data"] as? JSONObject,
              data["__typename"] as? String == "Playlist" else { return nil }
        ...
        return SpotifyPlaylist(
            ...
            images: parseGqlPlaylistImages(data["images"]),
            owner: SpotifyPlaylistOwner(id: ownerId, displayName: ownerData?["name"] as? String, uri: ownerData?["uri"] as? String),
            tracks: nil,
            uri: playlistUri,
            public: nil,
            collaborative: false,
...
// Nested JSON access helper
private func json(_ dict: JSONObject?, _ keys: String...) -> JSONObject? {
...
private func jsonString(_ dict: JSONObject?, _ keys: String...) -> String? {
    var current: Any? = dict
    for key in keys {
        guard let d = current as? JSONObject else { return nil }
        current = d[key]
...
private func jsonInt(_ dict: JSONObject?, _ keys: String...) -> Int? {
    var current: Any? = dict
    for key in keys {
        guard let d = current as? JSONObject else { return nil }
        current = d[key]
...
private func jsonArray(_ dict: JSONObject?, _ keys: String...) -> [Any]? {
    var current: Any? = dict
    for key in keys {
        guard let d = current as? JSONObject else { return nil }
        current = d[key]
...
private func parseJSONObject(from data: Data) throws -> JSONObject {
    let obj = try JSONSerialization.jsonObject(with: data, options: .fragmentsAllowed)
    guard let dict = obj as? JSONObject else {
        throw NSError(domain: "JSON", code: -1, userInfo: [NSLocalizedDescriptionKey: "Not a JSON object"])
    }
    return dict
}
